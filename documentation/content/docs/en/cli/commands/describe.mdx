...
| `--no-cache` | Disable caching, always fetch fresh data from AWS | `false` |
...
Batch `--describe` operations are automatically cached for 5 minutes. Repeated describe calls within this window return cached results instantly:
...
Cache is stored in `~/.config/cloudops-tools/inventory.db`
